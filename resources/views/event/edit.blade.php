    ...
    <div class="row">
        <div class="col-sm-12 col-lg-6">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('update.event',$post->event_id) }}"  method="POST" enctype="multipart/form-data" >
                        @csrf    
                        @method('PATCH') 
                        ...
                        <div class="form-group">
                            ...
                            <select name="id_kategori" class="form-select mb-3 shadow-none">
                                <option value="">Pilih Salah Satu</option>
                                @foreach ($kategori as $kategori)
                                @if ( old('id_kategori', $post->kategory_id) ==$kategori->id_kategori)
                        ...
                        <div class="form-group">
                            <label class="form-label" for="email1101">Nama Event :</label>
                            <input name="nama_event" type="text" class="form-control" id="text"  value="{{ old('nama_event', $post->nama_event) }}">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="email1101">Tanggal :</label>
                            <input name="tanggal_event" type="date" value="" class="form-control" id="exampleInputdate" value="{{ old('tanggal_event', $post->tanggal_event) }}">
                        </div>

                        <div class="form-group">
                            <label class="form-label" for="email1101">Jam :</label>
                            <input name="jam_event" type="time" class="form-control" id="exampleInputtime" value="{{ old('jam_event', $post->jam_event) }}" >
                        </div>

                        <div class="form-group">
                            <label class="form-label" for="email1101">Kuota :</label>
                            <input name="kuota_event" type="number" class="form-control" id="text"  value="{{ old('kuota_event', $post->kuota_event) }}">
                        </div>

                        <div class="form-group">
                            <label for="customFile1" class="form-label custom-file-input">Banner</label>
                            <input name="banner"  class="form-control" type="file" id="customFile1">
                        </div>

                        <div class="form-group">
                            <label class="form-label">Tempat Acara</label>
                            <select  name="id_ruang" class="form-select mb-3 shadow-none">
                                <option value="">Pilih Salah Satu</option>
                                @foreach ($ruang as $ruang)
                               

                                 @if ( old('id_ruang', $post->id_ruang) ==$ruang->id_ruang)
                                    <option value="{{ $ruang->id_ruang }}" selected>{{ $ruang->nama_ruang }}</option>
                                 @else
                        ...
                        <a href="{{ url('/admin/event')}}" class="btn btn-danger">Batal</a>
